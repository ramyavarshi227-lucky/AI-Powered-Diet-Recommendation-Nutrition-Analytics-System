...
def generate_pdf_diet_report(user_profile, calorie_target, diet_category, health_score, bmi_val, bmi_cat, meal_schedule, target_macros):
    ...
    title_style = ParagraphStyle('DocTitle', parent=styles['Title'], fontName='Helvetica-Bold', fontSize=18, textColor=colors.HexColor('#0B1020'), alignment=0, spaceAfter=4)
    ...
    heading_style = ParagraphStyle('Heading2', parent=styles['Heading2'], fontName='Helvetica-Bold', fontSize=13, textColor=colors.HexColor('#0B1020'), spaceBefore=12, spaceAfter=6)
    ...
    # Header & Branding
    story.append(Paragraph("AI-Powered Diet Recommendation & Nutrition Analytics System", title_style))
    ...
    p_data = [
        ...
        [Paragraph(f"<b>Personalization Score:</b> <font color='#00E676'><b>{pers_score}/100</b></font>", body_style), Paragraph("<b>Safety Shield:</b> <font color='#00E676'><b>VERIFIED ✓</b></font>", body_style)]
    ]
    ...
    story.append(Paragraph("Personalized 7-Meal Daily Schedule (Safety Verified)", heading_style))
    ...
    story.append(Paragraph("<b>Important Medical Disclaimer:</b>", ParagraphStyle('SubHeader', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=9, textColor=colors.HexColor('#FF9800'))))
    story.append(Paragraph("This system provides educational nutrition estimates and analytics and is not a medical diagnosis or treatment tool. Consult a qualified healthcare professional for medical or dietary decisions.", ParagraphStyle('Disc', parent=styles['Normal'], fontName='Helvetica-Oblique', fontSize=8, leading=10, textColor=colors.HexColor('#64748B'))))
...
def generate_pdf_weekly_plan(meal_schedule):
    ...
    styles = getSampleStyleSheet()
    title_style = ParagraphStyle('DocTitle', parent=styles['Title'], fontName='Helvetica-Bold', fontSize=18, textColor=colors.HexColor('#0B1020'), alignment=0)
    ...
    story.append(Paragraph("AI-Powered Diet Recommendation & Nutrition Analytics System", title_style))
    story.append(Paragraph("7-DAY WEEKLY MEAL MATRIX", sub_title_style))
    ...
    story.append(t_week)
    story.append(Spacer(1, 15))
    story.append(Paragraph("<i>Note: All 7 days are calibrated with portion scaling to meet daily caloric targets. Use 1-click Swap Meal in the application for daily variety.</i>", ParagraphStyle('Note', parent=styles['Normal'], fontName='Helvetica-Oblique', fontSize=8, textColor=colors.HexColor('#64748B'))))
...
def generate_pdf_grocery_list(categories_dict, budget_str):
    ...
    story.append(Paragraph("AI-Powered Diet Recommendation & Nutrition Analytics System", title_style))
    story.append(Paragraph(f"CATEGORIZED GROCERY CHECKLIST (EST. BUDGET: {budget_str})", sub_title_style))
